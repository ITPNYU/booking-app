...
const isPregameEvent = (title: string, description): boolean => {
  // Pregame events must have "Requester" in description and not start with square brackets
  const hasRequester =
    description &&
    typeof description === "string" &&
    description.includes("Requester");
  const notStartingWithBracket = !title?.startsWith("[");
  return Boolean(notStartingWithBracket && hasRequester);
};
  ...
  try {
    ...
    const resources =
      schema?.resources?.map((resource: any) => ({
        ...
        calendarIdProd: resource.calendarIdProd,
    ...
    for (const resource of resources) {
      ...
      //  break;
      //}
      console.log("calendarIdProd", resource.calendarIdProd);
      try {
        let pageToken: string | undefined;
        const calendarId = resource.calendarIdProd;
        do {
      ...
      return NextResponse.json(
        {
          ...
          summary: {
            ...
            newBookings: dryRunResults.filter(r => r.result === "new").length,
            existingBookings: dryRunResults.filter(r => r.result === "existing")
              .length,
