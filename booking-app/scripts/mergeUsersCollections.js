...
if (!admin.apps.length) {
  try {
    ...
    if (process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_PRIVATE_KEY) {
      ...
      console.log(
        "Service account credentials not found, trying application default credentials..."
      );
    ...
    console.error(
      "1. Create a .env.local file with FIREBASE_CLIENT_EMAIL and FIREBASE_PRIVATE_KEY"
    );
    console.error(
      '2. Run "firebase login" and "gcloud auth application-default login"'
    );
    process.exit(1);
  }
}
...
async function mergeUsersCollections(tenant = null) {
  try {
    ...
    const usersSuperAdminCollection = getTenantCollectionName(
      "usersSuperAdmin",
      tenant
    );
    const usersRightCollection = getTenantCollectionName(
      "mc-usersRights",
      tenant
    );
    ...
    usersAdminSnapshot.forEach((doc) => {
      ...
      if (!email) {
        console.warn(
          `Warning: Document ${doc.id} in usersAdmin has no email field`
        );
        return;
      }
    ...
    usersPaSnapshot.forEach((doc) => {
      ...
      if (!email) {
        console.warn(
          `Warning: Document ${doc.id} in usersPa has no email field`
        );
        return;
      }
    ...
    console.log(
      "Note: Super admin users are not stored in usersRights collection"
    );
    console.log(
      "They will be fetched directly from usersSuperAdmin collection"
    );
    const usersSuperAdminSnapshot = await db
      .collection(usersSuperAdminCollection)
      .get();
    ...
    usersSuperAdminSnapshot.forEach((doc) => {
      ...
      if (!email) {
        console.warn(
          `Warning: Document ${doc.id} in usersSuperAdmin has no email field`
        );
        return;
      }
      ...
      // They will be fetched directly from usersSuperAdmin collection
      console.log(
        `Skipping super admin user ${email} - will be fetched from original collection`
      );
      superAdminCount++;
    });

    console.log(
      `Processed ${superAdminCount} super admin users (not stored in usersRights)`
    );
    ...
    const adminUsers = Array.from(mergedUsers.values()).filter(
      (user) => user.isAdmin
    ).length;
    const liaisonUsers = Array.from(mergedUsers.values()).filter(
      (user) => user.isLiaison
    ).length;
    ...
    console.log(
      `- Super admin users: ${superAdminCount} (stored in original collection)`
    );
    console.log(
      `- Users with multiple roles: ${
        Array.from(mergedUsers.values()).filter(
          (user) => (user.isAdmin ? 1 : 0) + (user.isLiaison ? 1 : 0) > 1
        ).length
      }`
    );
  } catch (error) {
