...
export const processCancelBooking = async (
  ...
  if (guestEmail) {
    ...
    await serverSendBookingDetailEmail({
      calendarEventId: id,
      targetEmail: await getCancelCcEmail(tenant),
...
export const checkOut = async (id: string, email: string, tenant?: string) => {
  ...
  // Delegate checkout processing to /api/checkout-processing for all tenants
  try {
    const response = await fetch(
  ...
  // Auto-close: when XState transitions directly to Closed (e.g., ITP: Checked Out → Closed)
