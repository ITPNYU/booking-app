...
/**
 * Snapshot of the Media Commons `resources[].services` configs as stored in
 * the tenant schema (Firestore is the source of truth). Used only by test
 * fixtures (`testTenantSchemas.ts`, unit tests) so they render the same
 * schema-driven form as the real tenant. Regenerate from the dev schema when
