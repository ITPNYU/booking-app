...
const resolveDocPath = (ref: any) => {
  if (ref?.__type === "doc") {
    return {
      collectionPath: ref.pathParts.slice(0, -1).join("/"),
      docId: ref.pathParts.slice(-1)[0],
    };
  }
  if (Array.isArray(ref?.parts)) {
    const parts = ref.parts.filter((part) => typeof part === "string");
...
export const setDoc = async (docRef: any, data: Record<string, unknown>) => {
  ...
  const existingIndex = mockCollections[collectionPath].findIndex(
...
export const deleteDoc = async (docRef: any) => {
  const { collectionPath, docId } = resolveDocPath(docRef);
  if (!collectionPath || !docId || !mockCollections[collectionPath]) return;
  mockCollections[collectionPath] = mockCollections[collectionPath].filter(
    (doc) => doc.id !== docId,
  );
};
...
export const getDoc = async (docRef: any) => {
  ...
  const entry =
    collectionPath &&
    mockCollections[collectionPath]?.find((doc) => doc.id === docId);
  if (!entry) {
