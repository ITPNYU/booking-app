...
const ENDPOINT = `${BASE_URL}/api/bookings/auto-cancel-unapproved`;
const VALID_TOKEN = "test-cron-secret-for-e2e";

test.describe("Auto-cancel unapproved – cron API auth & dry-run", () => {
  ...
  test("200 dry-run with valid token", async ({ page }) => {
    ...
    expect(body).toHaveProperty("dryRunResults");
  });
});
